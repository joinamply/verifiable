...
function checkInputAttributes(input) {

    if (isNaN(parseFloat(input.val()))) {
        input.val(0);
    }

    if (input.attr("number-max")) {
        if (parseFloat(input.val()) > parseFloat(input.attr("number-max"))) {
            console.log('max');
            input.val(input.attr("number-max"));
        }
    }
}
